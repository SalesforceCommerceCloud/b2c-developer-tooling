...
class UserResetResource(BaseModel):
    ...
    supportTicketId: str | None = Field(
        None, description="Only required for users of the Salesforce organization."
    )
...
class Role(BaseModel):
    ...
    targetType: TargetType | None = Field(
        None, description="Type of entity this role can be assigned to."
    )
...
class Organization(BaseModel):
    ...
    contactUsers: list[str] | None = Field(
        None, description="List of contact user IDs."
    )
    realms: list[str] | None = Field(None, description="List of realm identifiers.")
    passwordMinEntropy: int | None = Field(
        None, description="Minimum password entropy requirement."
    )
    passwordHistorySize: int | None = Field(
        None, description="Number of previous passwords to remember."
    )
    passwordDaysExpiration: int | None = Field(
        None, description="Number of days until password expires."
    )
    sfAccountIds: list[str] | None = Field(
        None, description="Salesforce account identifiers."
    )
    type: Type | None = Field(None, description="Type of organization.")
    twoFARoles: list[str] | None = Field(
        None, description="List of role IDs that require two-factor authentication."
    )
    twoFAEnabled: bool | None = Field(
        None,
    ...
    sfMyDomainSuffix: str | None = Field(
        None, description="Salesforce My Domain suffix."
    )
    sfMyDomainVerified: bool | None = Field(
        None, description="Indicates if Salesforce My Domain is verified."
    )
    ...
    inactiveUserDays: int | None = Field(
        None, description="Number of days before a user is considered inactive."
    )
...
class FieldError(BaseModel):
    ...
    objectName: str | None = Field(
        None, description="Name of the object that failed validation."
    )
...
class UserCreate(BaseModel):
    ...
    firstName: str = Field(
        ..., description="First name of the user.", max_length=40, min_length=1
    )
    lastName: str = Field(
        ..., description="Last name of the user.", max_length=40, min_length=1
    )
    displayName: str | None = Field(
        None, description="Display name of the user.", max_length=100, min_length=1
    )
    ...
    preferredLocale: PreferredLocale | None = Field(
        None, description="Preferred locale for the user."
    )
    roles: list[str] | None = Field(
        None, description="List of IDs of the roles this user possesses."
    )
    organizations: list[str] = Field(
        ..., description="List of organization IDs this user belongs to."
    )
    primaryOrganization: str = Field(
...
class UserUpdate(BaseModel):
    ...
    firstName: str | None = Field(
        None, description="First name of the user.", max_length=40, min_length=1
    )
    lastName: str | None = Field(
        None, description="Last name of the user.", max_length=40, min_length=1
    )
    displayName: str | None = Field(
        None, description="Display name of the user.", max_length=100, min_length=1
    )
    ...
    primaryOrganization: str | None = Field(
        None, description="Primary organization ID for the user."
    )
    roleTenantFilter: str | None = Field(
        None,
        description="Filter for role tenant assignments. Format: ROLE_ENUM_NAME:instance_id,instance_id;ROLE_ENUM_NAME:instance_id\n- Role enum names are separated by semicolons (;)\n- Each role enum name is followed by a colon (:) and its tenant filters\n- Tenant filters are comma-separated (,)\n- Each tenant filter consists of a 4-character realm and 3-character instance_id separated by underscore (_)\n- A special case is an instance_id ending in _sbx, as it gives access to all sandboxes of a realm\n\nExample: CC_USER:aabc_prd,aabc_t12;LOGCENTER_USER:aamn_sbx\n",
...
class UserRead(BaseModel):
    ...
    passwordExpirationTimestamp: int | None = Field(
        None, description="Timestamp when the password expires."
    )
    passwordModificationTimestamp: int | None = Field(
        None, description="Timestamp of the last password modification."
    )
    ...
    lastLoginDate: date | None = Field(
        None, description="Date of the last successful login."
    )
    userState: UserState | None = Field(
        None, description="Current state of the user account."
    )
    ...
    verifiers: list[Verifier] | None = Field(
        None, description="List of authentication verifiers (e.g., MFA devices)."
    )
    deleteTimestamp: int | None = Field(
        None, description="Timestamp when the user was marked for deletion."
    )
    roleTenantFilterMap: dict[str, Any] | None = Field(
        None, description="Map of role tenant filter assignments."
    )
