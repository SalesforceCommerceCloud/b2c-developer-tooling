...
After a successful fallback, the tooling records that Account Manager rejected PKCE for that client ID and Account Manager host. Later CLI processes and VS Code extension activations start directly with the implicit flow and reuse its persisted access token instead of repeating a PKCE attempt that cannot succeed. The marker is stored with the authentication session (in the CLI session file or VS Code SecretStorage). `b2c auth logout` removes CLI sessions, while `SFCC_DISABLE_PKCE_FALLBACK=1` bypasses a remembered marker and forces PKCE so migration testing can surface the original failure.

On the first fallback, Account Manager may return a PKCE authorization code before rejecting the client at the token exchange. In that case the browser callback reports that authorization was received while the application logs the fallback warning and continues with implicit authentication. The browser callback no longer reports successful authentication before the application has a usable token.
