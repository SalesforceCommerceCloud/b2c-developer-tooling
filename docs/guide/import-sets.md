...
Import sets make B2C Commerce metadata deployments repeatable. They apply site archives in a predictable order and remember successful items on each target instance, so the same command can run during local setup, onboarding, and every CI/CD deployment.
...
All cartridge metadata is considered before project migrations. Cartridges are ordered by name, and ordered items within each source are sorted lexically.
...
Use a standard site-archive layout when all of the cartridge's initial metadata belongs together:
...
The `metadata/` directory is applied as one item.
...
Each immediate child directory or `.zip` file is one item. Keep a cartridge's `metadata/` directory in one form—either one site archive or an ordered collection—rather than mixing the two layouts.

Every directory item must contain at least one file. Empty directory trees are rejected before upload because they do not form an importable site archive.
...
Each immediate child directory or `.zip` file is one item. Hidden entries and other loose files are ignored. The directory can be omitted when cartridges provide all required imports.

## Naming Items

Use names in this form for every ordered child item:

```text
YYYYMMDDTHHmmss-description
```

Use UTC timestamps so ordering is consistent across time zones. Names are permanent once applied: do not modify or reuse an applied item. Add a new item with a later name for every subsequent change.
...
When an item needs manual follow-up, add a `README.md` or `README` at the top of that item directory. After importing, the CLI groups those instructions under **Post-import notes**. A dry run previews notes for pending items, while already-applied items do not repeat them.

This works for directory items in both cartridge `metadata/` sources and project migrations. Zip items are not inspected for notes. With `--json`, the note is included with the item result instead of being printed.

## Excluding Source Directories

Exclude project directories that contain fixtures, examples, archived checkouts, or other cartridges and migration items that must not participate in import sets. Each path is resolved from the project directory, and the directory and everything below it are ignored as import-set sources:
...
Exclusions select which directories can become import-set sources; they do not remove selected files from inside an archive item.

## How Repeat Runs Behave

After an item succeeds on an instance, later runs skip it. The item name—not its current contents—determines whether it has already run. This makes deployments fast and repeatable, but it also means editing an applied item can make instances diverge.

If an item fails, the command stops. Fix the archive or target-instance problem and run the command again; earlier successful items are skipped.

A run interrupted at exactly the wrong time can retry its current item. Design every archive so applying it more than once is safe.

## Teams and CI/CD

The default import history is shared across developers, checkouts, and CI runners targeting the same instance. Concurrent runs coordinate automatically: one applies items while the others wait, then waiting runs skip completed work.
...
Use a new set ID to leave the existing history intact while treating every current item as pending:
...
For a custom set ID, replace the final `migrations` path segment with that ID. Clearing a history permanently forgets which items succeeded, so the next run treats every current item as pending. Use this approach only when every archive is safe to apply again.
