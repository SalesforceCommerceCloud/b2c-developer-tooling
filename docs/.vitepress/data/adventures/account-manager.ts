...
function bmAdminOcapi(clientId: string): string {
  const resources = [
...
export const accountManagerAdventure = defineAdventure({
  id: 'account-manager',
  ...
  priority: 'common',
  intro:
    'Account Manager covers cross-instance identity (org users, API clients, orgs). Business Manager admin covers per-instance roles, users, and access keys via the OCAPI Data API. Pick the target you need — some commands are user-auth only, and `bm access-key` writes need an extra BM functional permission.',
  ...
  steps: [
    step('target', {
      ...
      choices: [
        choice('am', {
          ...
          body: md`Cross-instance identity: \`b2c am users\`, \`b2c am roles\`, \`b2c am clients\`, \`b2c am orgs\`.`,
        ...
        choice('bm', {
          ...
          body: md`Instance-scoped admin via OCAPI Data API: \`b2c bm roles\`, \`b2c bm users\`, \`b2c bm access-key\`, \`b2c bm whoami\`.`,
        ...
        choice('both', {
          ...
          body: md`Set up credentials that work across Account Manager and Business Manager admin commands.`,
    ...
    step('auth', {
      ...
      doc: doc('/guide/authentication', 'authentication-methods', 'Authentication Methods'),
      choices: [
        choice('user-auth', {
          ...
          body: md`Required for \`am clients create/update/delete\`, \`am orgs\`, \`bm whoami\`, and \`bm access-key\`. Uses your real user roles and BM functional permissions.`,
        ...
        choice('client-credentials', {
          ...
          body: md`Service-account token. Works for read-only AM (\`users list\`, \`roles list\`) and most BM \`roles\`/\`users\` commands once OCAPI is configured.`,
        ...
        choice('jwt', {
          ...
          body: md`Cert pair instead of a client secret. See [JWT setup](/guide/authentication#jwt-authentication-certificate-based).`,
    ...
    step('ocapi', {
      ...
      showIf: (state: AdventureState) => state.target === 'bm' || state.target === 'both',
      doc: doc('/guide/authentication', 'minimal-configuration-by-feature', 'Minimal Configuration by Feature'),
      choices: [
        choice('yes', {
          ...
          badges: [{text: 'Recommended', tone: 'info'}],
          body: md`Emits a JSON block to paste into Business Manager → Site Development → Open Commerce API Settings → Data API.`,
        ...
        choice('no', {
          ...
          body: md`\`bm whoami\` and \`bm access-key\` use user-auth and don't need OCAPI Data API entries beyond \`/users/this\` and \`/users/*/access_key/*\`.`,
    ...
    step('persistence', {
      ...
      choices: [
        ...
        choice('env', {
          ...
          body: md`Use \`SFCC_*\` environment variables — the CLI auto-loads \`.env\` files.`,
        ...
        choice('stateful', {
          ...
          body: md`Best for interactive use — sign in once and reuse the session across \`am\` and \`bm\` commands until it expires.`,
