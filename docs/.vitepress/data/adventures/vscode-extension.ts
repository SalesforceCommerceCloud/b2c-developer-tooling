...
// The synthesizer assembles a tailored dw.json based on the chosen feature
// mix — the extension reuses whatever the CLI uses, so the credentials
// surface is feature-driven, not connection-driven.
...
export const vscodeExtensionAdventure = defineAdventure({
  id: 'vscode-extension',
  ...
  priority: 'core',
  intro:
    'The B2C DX VS Code Extension is in Developer Preview and ships as a pre-built VSIX from GitHub releases. It reuses your existing CLI configuration, so once your dw.json (or SFCC_* env vars) is in place, every feature you turn on picks up the same connection.',

  steps: [
    step('project', {
      title: 'What kind of project?',
      doc: doc('/vscode-extension/', undefined, 'VS Code Extension overview'),
      choices: [
        choice('cartridges', {
          ...
          body: md`Server-side cartridges with WebDAV upload, code-version management, and the Script Debugger.`,
        ...
        choice('headless', {
          ...
          body: md`Frontend-only project — the extension is mostly useful for the **SCAPI API Browser** and **Sandbox Realm Explorer**.`,
        ...
        choice('both', {
          ...
          body: md`Mixed workspace — pick all the features you actually use below.`,
    ...
    step('features', {
      ...
      multiSelect: true,
      minPicks: 1,
      doc: doc('/vscode-extension/', 'highlights', 'Extension Highlights'),
      choices: [
        choice('sandbox', {
          ...
          badges: [{text: 'Recommended', tone: 'info'}],
          body: md`Spin up, start, stop, and clone on-demand sandboxes from a tree view. Needs an OAuth client with the \`Sandbox API User\` role.`,
        ...
        choice('codeSync', {
          ...
          body: md`Auto-upload cartridges as you save. Uses WebDAV + OCAPI for code-version operations.`,
        ...
        choice('libraries', {
          ...
          body: md`Browse, edit, and round-trip Page Designer libraries. Reads library XML over WebDAV.`,
        ...
        choice('debugger', {
          ...
          body: md`Step through controllers, jobs, hooks, SCAPI hooks, and Custom APIs on the sandbox.`,
        ...
        choice('webdav', {
          ...
          body: md`Browse \`Catalogs/\`, \`Libraries/\`, and \`IMPEX/\` folders inside VS Code.`,
        ...
        choice('logs', {
          ...
          body: md`Stream \`error-*.log\`, \`warn-*.log\`, and \`info-*.log\` into an output channel.`,
        ...
        choice('apiBrowser', {
          ...
          icon: 'mdi:api',
          body: md`Explore SCAPI APIs and run authenticated requests. Needs \`short-code\` + \`tenant-id\`.`,
    ...
    step('config', {
      ...
      doc: doc('/vscode-extension/configuration', 'connecting-to-a-b2c-instance', 'Connecting to a B2C Instance'),
      choices: [
        choice('dw-json', {
          ...
          badges: [{text: 'Recommended', tone: 'info'}],
          body: md`Per-project file the extension and CLI both read. Walk-up discovery from the workspace root.`,
        ...
        choice('env', {
          ...
          icon: 'mdi:console-line',
          body: md`Use \`SFCC_*\` env vars — VS Code picks them up from the launching shell.`,
