...
export const mrtDeployAdventure = defineAdventure({
  id: 'mrt-deploy',
  ...
  intro:
    "Managed Runtime commands talk to the MRT control plane with an API key (separate from Account Manager OAuth). You'll pick what you're deploying, where the API key lives, and whether deploys are run by hand or by CI.",

  steps: [
    step('project-type', {
      title: 'What are you deploying?',
      subtitle: 'Both project types use the same b2c mrt commands — only the build artifacts differ.',
      doc: doc('/cli/mrt', 'command-overview', 'MRT command overview'),
      choices: [
        choice('pwa-kit', {
          title: 'PWA Kit v3',
          ...
          body: md`Build with \`npm run build\`; deploy the \`build/\` directory with \`b2c mrt bundle deploy\`.`,
        ...
        choice('storefront-next', {
          ...
          body: md`Build the Storefront Next app, then deploy its build output with \`b2c mrt bundle deploy\`. Access is currently limited to pilot customers — see [Storefront Next on Managed Runtime](/guide/storefront-next) for the full setup.`,
        ...
        choice('both', {
          ...
          body: md`Manage PWA Kit and Storefront Next under the same MRT account — one project per app, deployed independently.`,
    ...
    step('credentials', {
      ...
      doc: doc('/guide/authentication', 'managed-runtime-api-key', 'Managed Runtime API Key'),
      choices: [
        choice('mobify', {
          title: '~/.mobify file',
          ...
          body: md`Run \`b2c mrt save-credentials --user <email> --api-key <KEY>\` once. The CLI reads \`~/.mobify\` for every subsequent \`b2c mrt\` call.`,
        ...
        choice('env', {
          ...
          body: md`Export \`MRT_API_KEY\` (and optionally \`MRT_PROJECT\` / \`MRT_ENVIRONMENT\`) from your shell or your runner's secret store. Best for CI/CD.`,
        ...
        choice('flags', {
          ...
          body: md`Pass credentials and the target on every invocation. Useful when juggling multiple MRT accounts in one shell.`,
    ...
    step('trigger', {
      ...
      doc: doc('/cli/mrt', 'b2c-mrt-bundle-deploy', 'b2c mrt bundle deploy'),
      choices: [
        choice('manual', {
          title: 'Manual',
          subtitle: 'Developer runs it locally',
          icon: 'mdi:account-hard-hat-outline',
          body: md`You build the bundle and run \`b2c mrt bundle deploy\` from your laptop. Add \`--wait\` to block until the deployment finishes.`,
        ...
        choice('ci', {
          ...
          body: md`A pipeline builds the storefront and runs \`b2c mrt bundle deploy\` with credentials from the runner's secret store. See the example workflow in the warnings panel.`,
