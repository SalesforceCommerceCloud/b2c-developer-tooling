...
> Default auth-methods chain is `['client-credentials', 'jwt', 'user']`. `am`/`ods` commands
> default the client id to the public client `a40a7a9b…`; base `auth login`/`auth token` do **not**
> (they require an explicit client id). `bm whoami` is the one command that defaults to `['user']`.
...
2. **Expect:** PKCE token exchange fails → a single **WARN**, then automatic implicit login in the
   ...
   > `[Auth] Authorization Code + PKCE failed for client <id> (<oauth error>). Falling back to the deprecated implicit flow. Re-register this client as a public (PKCE) client in Account Manager to remove this warning.`
...
4. **Negative — must NOT fall back** (plain errors, not `PkceGrantUnsupportedError`):
   - **User cancels** the browser consent → error, no implicit retry.
