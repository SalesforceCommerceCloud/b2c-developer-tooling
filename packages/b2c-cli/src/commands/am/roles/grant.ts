 ...
 */
export default class RoleGrant extends AmCommand<typeof RoleGrant> {
  static args = {
    login: Args.string({
  ...
  async run(): Promise<AccountManagerUser> {
    ...
    if (!user) {
      this.error(t('commands.role.grant.notFound', 'User {{login}} not found', {login}));
    }
    ...
    const updatedUser = await this.accountManagerClient.grantRole(user.id, role, scope);
