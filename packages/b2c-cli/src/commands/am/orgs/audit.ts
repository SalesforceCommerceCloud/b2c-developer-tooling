 ...
 */
export default class OrgAudit extends AmCommand<typeof OrgAudit> {
  static args = {
    org: Args.string({
  ...
  async run(): Promise<AuditLogCollection> {
    ...
    let organization;
    try {
      organization = await this.accountManagerClient.getOrg(org);
    } catch (error) {
      // If not found by ID, try by name
      if (error instanceof Error && error.message.includes('not found')) {
        try {
          organization = await this.accountManagerClient.getOrgByName(org);
        } catch {
    ...
    const result = await this.accountManagerClient.getOrgAuditLogs(organization.id);
