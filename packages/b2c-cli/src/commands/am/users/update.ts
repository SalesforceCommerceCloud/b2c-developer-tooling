 ...
 */
export default class UserUpdate extends AmCommand<typeof UserUpdate> {
  static args = {
    login: Args.string({
  ...
  async run(): Promise<AccountManagerUser> {
    ...
    if (!user) {
      this.error(t('commands.user.update.notFound', 'User {{login}} not found', {login}));
    }
    ...
    const updatedUser = await this.accountManagerClient.updateUser(user.id, changes as UserUpdateType);
