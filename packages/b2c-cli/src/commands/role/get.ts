...
import {getRole} from '@salesforce/b2c-tooling-sdk/operations/roles';
 ...
 */
export default class RoleGet extends RoleCommand<typeof RoleGet> {
  static args = {
    roleId: Args.string({
      description: 'Role ID',
      required: true,
  ...
  async run(): Promise<AccountManagerRole> {
    ...
    const role = await getRole(this.accountManagerRolesClient, roleId);
