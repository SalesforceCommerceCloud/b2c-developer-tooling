...
describe('Sites Operations E2E Tests', function () {
  ...
  before(async function () {
    ...
    let importResult = await runCLI(['job', 'import', SITE_ARCHIVE_PATH, '--server', serverHostname]);
    for (let attempt = 1; attempt <= 2 && importResult.exitCode !== 0; attempt++) {
      const msg = importResult.stderr || importResult.stdout;
      if (!/fetch failed|ECONNRESET|ETIMEDOUT/i.test(msg)) {
        break;
      }
      console.warn(
        `Sites E2E: transient import error (attempt ${attempt}), retrying after delay:`,
        msg,
      );
      await sleep(2_000);
      importResult = await runCLI(['job', 'import', SITE_ARCHIVE_PATH, '--server', serverHostname]);
    }
