...
import EcdnRateLimitGet from '../../../../src/commands/ecdn/rate-limit/get.js';
import {createIsolatedConfigHooks, createTestCommand, runSilent} from '../../../helpers/test-setup.js';

describe('ecdn rate-limit get', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnRateLimitGet, hooks.getConfig(), flags, {});
  }
  ...
  it('returns rule in JSON mode', async () => {
  ...
  it('returns rule in non-JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
      'rule-id': 'rule-123',
