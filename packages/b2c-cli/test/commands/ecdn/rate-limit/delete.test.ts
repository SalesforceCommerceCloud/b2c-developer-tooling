...
import EcdnRateLimitDelete from '../../../../src/commands/ecdn/rate-limit/delete.js';
import {createIsolatedConfigHooks, createTestCommand} from '../../../helpers/test-setup.js';

describe('ecdn rate-limit delete', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnRateLimitDelete, hooks.getConfig(), flags, {});
  }
  ...
  it('requires force flag in non-JSON mode', async () => {
    const command: any = await createCommand({'tenant-id': 'zzxy_prd', zone: 'my-zone', 'rule-id': 'rule-123'});
    stubCommon(command, {jsonEnabled: false});

    const result = await command.run();

    expect(result).to.have.property('deleted', false);
    expect(result).to.have.property('ruleId', 'rule-123');
  });

  it('deletes with force flag in JSON mode', async () => {
