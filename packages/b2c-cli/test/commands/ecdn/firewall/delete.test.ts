...
import EcdnFirewallDelete from '../../../../src/commands/ecdn/firewall/delete.js';
import {createIsolatedConfigHooks, createTestCommand} from '../../../helpers/test-setup.js';

describe('ecdn firewall delete', () => {
  ...
  async function createCommand(flags: Record<string, unknown> = {}) {
    return createTestCommand(EcdnFirewallDelete, hooks.getConfig(), flags, {});
  }
  ...
  it('requires force flag in non-JSON mode', async () => {
    ...
    expect(result).to.have.property('ruleId', 'rule-1');
  });

  it('deletes with --force in JSON mode', async () => {
    const command: any = await createCommand({
      'tenant-id': 'zzxy_prd',
      zone: 'my-zone',
      'rule-id': 'rule-1',
