...
describe('docs: standard job steps corpus', () => {
  ...
  it('reads a job step doc with purpose and parameter table', () => {
    const content = readDoc('ImportCatalog');
  ...
  it('renders the Scope field on every step doc', () => {
    for (const step of dataset.steps) {
      const content = readDoc(step.typeId);
  ...
  it('resolves a job step via readDocByQuery (cross-corpus file resolution)', () => {
    const result = readDocByQuery('ExportInventoryLists');
