...
export function renderMarkdown(input: string): string {
  ...
  for (const rawLine of lines) {
    ...
    // Fenced code blocks. When consecutive code blocks are separated only
    // by blank lines (the renderer's blank-line rule fires between them),
    // we merge them into a single <pre> with newline separators so they
    // render as a tight stanza — no per-block padding stacking.
    const fence = line.match(/^```(\w*)\s*$/);
    if (fence) {
      if (inCodeBlock) {
        const html = `<pre><code${codeLang ? ` class="language-${escapeHtml(codeLang)}"` : ''}>${escapeHtml(
          codeBuffer.join('\n'),
        )}</code></pre>`;
        // If the previous emission was a <pre> (i.e. nothing in between but
        ...
        if (last && last.startsWith('<pre>') && last.endsWith('</code></pre>')) {
          // Reuse the previous <pre>'s opening; concatenate inner content
          // separated by a blank-line so commands stay readable.
          const merged =
            last.slice(0, last.length - '</code></pre>'.length) +
            '\n' +
