...
Fixed a Managed Runtime notifications bug where creating or updating a notification failed unless all three deployment-event flags were provided. `mrt project notification create` and `update` now always send `deployment_start`, `deployment_success`, and `deployment_failed`; a flag you don't pass is sent as off. Note this means `update` no longer partially updates the event flags — set the ones you want on every update, or an unspecified event is turned off.
